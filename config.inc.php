...
#	'query_advisor'	=> '/usr/bin/pt-query-advisor',
    ...
    /*
		 Changes made by Tim Langhans, Qiiwi Interactive to match
		 Backpackers AWS Aurora production database. Environment Variables
		 have to be set on machine running this application and eventually
		 even forwarded to server environment, e.g. for Apache httpd.
		 */
    $conn['host'] = getenv('BP_HOST');
    $conn['port'] = getenv('BP_PORT');
		$conn['user'] = getenv('BP_USER');
		$conn['password'] = getenv('BP_PASS');
		$conn['db'] = getenv('BP_DB');
		return $conn;
	},
);
